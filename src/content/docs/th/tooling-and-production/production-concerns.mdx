...
มีข้อควรระวังตอนอ่านการแยกนี้บน desktop: ตอนนี้ Flutter **รวม UI thread กับ raster thread เข้าด้วยกัน** บน macOS และ Windows (ตั้งแต่ 3.35) และบน Linux (ตั้งแต่ 3.41) สองเฟสยังทำงานตามลำดับเหมือนเดิม แต่ไม่ได้รันขนานกันคนละ thread แล้ว ดังนั้น cost ของ raster จะโผล่มาในรูปของเวลาที่ UI thread เอาไป build ไม่ได้ ส่วนฝั่ง mobile ยังมี raster thread แยกอยู่ ไม่ว่าทางไหน budget ก็เท่าเดิม — ทั้ง frame มีเวลา ~16ms
