...
scrolling สร้างบน **lazy building** — build เฉพาะ item ใกล้ viewport แล้ว recycle — ดังนั้นใช้ `ListView.builder` เสมอสำหรับอะไรที่อาจยาว **viewport** เก็บ **sliver** ที่เป็นพื้นที่ scroll ได้ที่ layout แบบ lazy; ประกอบด้วย `CustomScrollView` (เช่น `SliverAppBar` ที่ collapse เหนือ `SliverList`) เมื่อ scroll view เดียวต้องการหลาย effect เลี่ยง `shrinkWrap` บน list ใหญ่ เพราะทิ้ง laziness ไปหมด
</Callout>
...
export const quizSlivers = [
  {
    ...
    options: [
      'ดูสวยกว่า',
      'build child แบบ lazy ตามความต้องการ แทนที่จะ build ทั้งหมดตั้งแต่แรก',
      'จำเป็นสำหรับทุก list',
      'ปิด scrolling',
    ],
    answer: 1,
  },
  ...
  },
  {
    ...
    options: [
      'เปลี่ยนสี',
      'build child ทั้งหมด (เสีย laziness) เพื่อ size list ตาม content',
      'ปิดการ tap',
      'ไม่มีข้อเสีย',
    ],
    answer: 1,
