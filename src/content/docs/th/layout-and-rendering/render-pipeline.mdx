...
แต่ละ frame รัน **build → layout → paint → composite** ภายใน budget ~16ms; jank มักมาจากงานเยอะเกินใน build หรือ layout widget เป็นคำอธิบายที่เบาและ immutable; **RenderObject** ที่แพง (tree ที่สาม) ถูก mutate ในที่เดิม นั่นคือเหตุผลที่ rebuild widget ถูก wrap subtree ที่ repaint บ่อยด้วย **`RepaintBoundary`** เพื่อให้ repaint อยู่บน layer ของตัวเอง — และ profile ด้วย DevTools แทนการเดา
</Callout>
...
export const quizPipeline = [
  ...
  },
  {
    ...
    options: [
      ...
      'เร่ง layout ด้วยการข้าม layout ไปเลย',
