...
One caveat when reading that split on desktop: Flutter now **merges the UI and raster threads** on macOS and Windows (since 3.35) and on Linux (since 3.41). The two phases still happen in order, but they no longer run concurrently on separate threads, so raster cost shows up as time the UI thread cannot spend building. Mobile keeps the separate raster thread. Either way the budget is the same — the frame has ~16ms total.
